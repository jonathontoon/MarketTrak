    //
//  MTSteamInventoryCommunicator.swift
//  MarketTrak
//
...
@objc protocol MTSteamInventoryCommunicatorDelegate {
...
class MTSteamInventoryCommunicator: NSObject {
    
    let managedObjectContext = (UIApplication.sharedApplication().delegate as! AppDelegate).managedObjectContext
    
    var delegate: MTSteamInventoryCommunicatorDelegate!
    ...
    func getItemsInUserInventory(user: MTUser) {
        
        let inventoryURL = "https://steamcommunity.com/profiles/\(user.steamID64)/inventory/json/730/2"
        
        getJSONFromURL(url: inventoryURL, withCompletion: {
            ...
            if error == nil {
                if let d = data {
                    ...
                    for d in json["rgDescriptions"] {
                        
                        let item = d.1
                        
                        if item["tradable"].intValue == 1 {
                            
                            let fullName = item["market_name"].stringValue

                            let inventoryItem = MTItem()

                            // Item URL
//                            if let itemURL = node["href"] {
//                                inventoryItem.itemURL = NSURL(string: itemURL + "?l=english&country=us")
//                            }
...
//                            inventoryItem.currentPrice = MTCurrencyItem(currency: NSNumber(float: Float(priceString)!))

                            // Quantity
                            //inventoryItem.quantity = Int(node.css("span.market_listing_num_listings_qty").text!.stringByReplacingOccurrencesOfString(",", withString: ""))

                            // Weapon
                            inventoryItem.weaponType = determineWeapon(fullName)

                            // Item Name
                            inventoryItem.name = determineItemName(fullName)
                            
                            // Type
                            inventoryItem.type = determineType(fullName)
                        
                            // Category
                            inventoryItem.category = determineCategory(fullName)
                            
                            // Exterior
                            inventoryItem.exterior = determineExterior(fullName)
